...
class JobController {
    ...
    static async postJob(req, res) {
        ...
        try {
            ...
            const job = await Job.create({ title, description, companyId, authorId: req.user.id, jobType }, { transaction })
            const newSkill = skills.map(el => {
                el.jobId = job.id
                el.name
                el.level
                return el
            })
            await Skill.bulkCreate(newSkill, { transaction })
